...
public class Old_SendingTask
{
	...
	public Old_SendingTask (String sourcePath, String ip, int port, Logger logger, ExecutorService executorService)
	{
	...
	private void searchAndSend(String path)
	{
		...
		if (contents.length == 0)
		{
			...
			executorService.submit(new Runnable()
			{
				...
				public void run()
				{
					Old_SendingFile sending = new Old_SendingFile(path, ip, port, source, logger);
					sending.sendTask();
			...
			if(!file.isDirectory())
			{
				Runnable task = new Runnable()
				{
					...
					public void run()
					{
						...
						// 파일 전송
						Old_SendingFile sending = new Old_SendingFile( getShortPath( path, file.getName() )
													, ip, port, file, logger);
