...
SELECT T1.EMPNO, T1.HIREDATE, T1.WORK_DAY
FROM
(
...
WHERE T1.WORK_DAY = (SELECT MAX(TO_DATE('2006/08/05', 'YYYY/MM/DD') - HIREDATE)
                     FROM EMP);

--5번
WITH T AS (SELECT EMPNO, HIREDATE, TO_DATE('2006/08/05', 'YYYY/MM/DD') - HIREDATE "WORK_DAY"
             FROM EMP)
SELECT T.EMPNO, T.HIREDATE, T.WORK_DAY
FROM T
WHERE WORK_DAY = (SELECT MAX(T.WORK_DAY) FROM T) 
   OR WORK_DAY = (SELECT MIN(T.WORK_DAY) FROM T);
